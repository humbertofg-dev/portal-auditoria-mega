...
dashboardRouter.get("/cards", async (req, res) => {
  ...
  const tempoMedioRespostaAuditoria = calcularMediaHoras(
    justificativasComTempos.map((j) => [j.createdAt, j.dataPrimeiraAnalise!])
  );
  ...
  res.json({
    ...
    lojasComOcorrencias: lojasComOcorrencias.filter((l) => l.lojaId).length,
    produtosAjustados: produtosAjustados.filter((p) => p.produtoCodigo).length,
...
dashboardRouter.get("/top-lojas", async (req, res) => {
  ...
  const lojas = await prisma.loja.findMany({
    where: { id: { in: agrupado.map((g) => g.lojaId!).filter(Boolean) } },
  });
...
dashboardRouter.get("/motivos", async (req, res) => {
  ...
  res.json(agrupado.map((g) => ({ motivo: g.motivo, quantidade: g._count.id })));
});
...
dashboardRouter.get("/heatmap-regional", async (req, res) => {
  ...
  const resultado = await Promise.all(
    regionais.map(async (regional) => {
      const lojaIds = regional.lojas.map((l) => l.id);
...
dashboardRouter.get("/ranking/:tipo", async (req, res) => {
  ...
  if (tipo === "regionais") {
    const regionais = await prisma.regional.findMany({ include: { lojas: true } });
    const resultado = await Promise.all(
      regionais.map(async (r) => {
        const lojaIds = r.lojas.map((l) => l.id);
