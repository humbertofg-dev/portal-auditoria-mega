...
// Remove a justificativa e todos os registros derivados dela (histórico,
// análises, comentários, alertas vinculados), preservando rastreabilidade
// através de um LogAcesso com os dados principais do registro excluído —
// já que depois da exclusão não há mais como consultar a Justificativa em si.
justificativasRouter.delete(
  "/:id",
  autorizar([PerfilUsuario.ADMINISTRADOR]),
  async (req, res) => {
    const justificativa = await prisma.justificativa.findUnique({ where: { id: req.params.id } });
    ...
    await prisma.$transaction([
      prisma.comentario.deleteMany({ where: { justificativaId: justificativa.id } }),
      prisma.historicoStatus.deleteMany({ where: { justificativaId: justificativa.id } }),
      prisma.analiseAuditoria.deleteMany({ where: { justificativaId: justificativa.id } }),
      prisma.alerta.deleteMany({ where: { justificativaId: justificativa.id } }),
      prisma.justificativa.delete({ where: { id: justificativa.id } }),
    ]);

    await prisma.logAcesso.create({
      data: {
        usuarioId: req.usuario!.sub,
        acao: "EXCLUIU_JUSTIFICATIVA",
        entidade: "Justificativa",
        entidadeId: justificativa.id,
        detalhes: JSON.stringify({
          lojaCodigoBruto: justificativa.lojaCodigoBruto,
          motivo: justificativa.motivo,
          valorAjustado: justificativa.valorAjustado,
          dataEnvio: justificativa.dataEnvio,
