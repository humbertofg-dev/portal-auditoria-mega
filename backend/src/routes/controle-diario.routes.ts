...
import {
  reconciliarJustificativasExistentes,
} from "../services/controle-diario.service";
...
controleDiarioRouter.post(
  ...
  async (req, res) => {
    ...
    // Reconcilia justificativas que já existiam no banco ANTES da geração
    // do controle (ex: lojas que responderam antes do controle ser criado).
    // Roda em background para não atrasar a resposta da API.
    reconciliarJustificativasExistentes(controle.id).then(({ atualizados }) => {
      if (atualizados > 0) {
        logger.info(`Reconciliação pós-geração: ${atualizados} item(s) atualizados automaticamente.`);
      }
    }).catch(() => {});
...
// --- Reconciliação manual (força atualização dos itens pendentes) ---
// Útil quando o controle foi criado ANTES das respostas chegarem, ou quando
// respostas antigas precisam ser vinculadas retroativamente.
controleDiarioRouter.post(
  "/:id/reconciliar",
  autorizar([PerfilUsuario.ADMINISTRADOR, PerfilUsuario.AUDITORIA]),
  async (req, res) => {
    const controle = await prisma.controleDiario.findUnique({ where: { id: req.params.id } });
    if (!controle) return res.status(404).json({ erro: "Controle não encontrado." });
    if (controle.fechado) return res.status(409).json({ erro: "Controle já fechado." });

    const { atualizados } = await reconciliarJustificativasExistentes(req.params.id);
    res.json({ atualizados, mensagem: `${atualizados} item(s) atualizado(s).` });
  }
);

// --- Adicionar lojas a um controle já existente ---
controleDiarioRouter.post(
  "/:id/adicionar-lojas",
  autorizar([PerfilUsuario.ADMINISTRADOR, PerfilUsuario.AUDITORIA]),
  async (req, res) => {
    const { lojaIds } = req.body as { lojaIds: string[] };
    if (!lojaIds?.length) return res.status(400).json({ erro: "Nenhuma loja informada." });

    const controle = await prisma.controleDiario.findUnique({ where: { id: req.params.id } });
    if (!controle) return res.status(404).json({ erro: "Controle não encontrado." });
    if (controle.fechado) return res.status(409).json({ erro: "Controle já fechado." });

    const existentes = await prisma.itemControleDiario.findMany({
      where: { controleDiarioId: controle.id },
      select: { lojaId: true },
    });
    const idsExistentes = new Set(existentes.map((i: { lojaId: string }) => i.lojaId));
    const novas = lojaIds.filter((id: string) => !idsExistentes.has(id));

    if (novas.length > 0) {
      await prisma.itemControleDiario.createMany({
        data: novas.map((lojaId: string) => ({ controleDiarioId: controle.id, lojaId })),
      });
      await reconciliarJustificativasExistentes(controle.id);
    }

    const atualizado = await prisma.controleDiario.findUnique({
      where: { id: controle.id },
      include: {
        itens: { include: { loja: { include: { regional: true } } }, orderBy: { loja: { nome: "asc" } } },
        criadoPor: { select: { id: true, nome: true } },
...
// --- Remover loja de um controle ---
controleDiarioRouter.delete(
  "/item/:itemId",
  autorizar([PerfilUsuario.ADMINISTRADOR, PerfilUsuario.AUDITORIA]),
  async (req, res) => {
    const item = await prisma.itemControleDiario.findUnique({ where: { id: req.params.itemId } });
    ...
    const controle = await prisma.controleDiario.findUnique({ where: { id: item.controleDiarioId } });
    if (controle?.fechado) return res.status(409).json({ erro: "Controle já fechado." });

    await prisma.itemControleDiario.delete({ where: { id: req.params.itemId } });
    res.status(204).send();
  }
);
