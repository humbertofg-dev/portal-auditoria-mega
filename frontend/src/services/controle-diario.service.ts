...
export async function reconciliarManualmente(
  controleId: string
): Promise<{ atualizados: number; mensagem: string }> {
  const { data } = await api.post(`/controle-diario/${controleId}/reconciliar`);
  return data;
}
