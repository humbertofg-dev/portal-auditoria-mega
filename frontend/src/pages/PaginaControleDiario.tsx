...
export function PaginaControleDiario() {
  ...
  const [msgReconciliacao, setMsgReconciliacao] = useState<string | null>(null);
  ...
  async function reconciliar() {
    ...
    setMsgReconciliacao(null);
    try {
      const { atualizados } = await controleDiarioService.reconciliarManualmente(controle.id);
      setMsgReconciliacao(
        atualizados > 0
          ? `✅ ${atualizados} loja(s) atualizadas automaticamente.`
          : "Nenhuma justificativa pendente encontrada para reconciliar."
      );
      recarregar();
      recarregarDashboard();
    } catch {
      setMsgReconciliacao("Erro ao reconciliar. Tente novamente.");
    } finally {
    ...
    <div className="flex flex-col gap-5">
      ...
      {abaAtiva === "historico" ? (
        ...
        <TabControleDiario
          ...
          msgReconciliacao={msgReconciliacao}
        />
      )}
    </div>
  );
      ...
      {controle && (
        <>
          ...
          <div className="flex flex-col gap-2">
            <div className="flex items-center gap-3">
              ...
              {podeOperar && !controle.fechado && (
                <button
                  ...
                  className="flex items-center gap-1.5 rounded-md border border-mega-gray-200 bg-white px-3.5 py-2 text-sm font-medium text-mega-gray-700 hover:bg-mega-gray-50 disabled:opacity-60"
                  title="Força a vinculação de respostas do Forms que já chegaram mas ainda não foram detectadas"
                >
                  <RefreshCw size={14} className={reconciliando ? "animate-spin" : ""} />
                  {reconciliando ? "Verificando…" : "Reconciliar Agora"}
                </button>
              )}
              {podeOperar && !controle.fechado && (
                <button
            ...
            {msgReconciliacao && (
              <p className={`text-xs px-1 ${msgReconciliacao.startsWith("✅") ? "text-emerald-600" : "text-mega-gray-500"}`}>
                {msgReconciliacao}
              </p>
            )}
          </div>
