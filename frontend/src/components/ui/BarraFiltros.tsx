import { FiltrosGlobais, Regional, Loja, StatusJustificativa } from "@/types";
...
export function BarraFiltros({ filtros, onChange }: BarraFiltrosProps) {
  // Busca as próprias listas de apoio (regionais e lojas) uma única vez,
  // já que o componente é usado em várias telas (Dashboard, Justificativas,
  // Indicadores, Produtos, Relatórios) — assim cada tela não precisa
  // duplicar essa lógica de carregamento.
  ...
  const { dados: lojas } = usePolling<Loja[]>(
    async () => {
  ...
  // Ao escolher uma loja, detecta automaticamente a regional dela e já
  // preenche o filtro de Regional junto — evita o usuário ter que selecionar
  // os dois filtros manualmente quando já sabe a loja que procura.
  function selecionarLoja(lojaId: string) {
    if (!lojaId) {
      atualizar("lojaId", undefined);
      return;
    }
    ...
    onChange({
      ...filtros,
      lojaId,
      regionalId: loja?.regionalId ?? filtros.regionalId,
    });
  }
  ...
  return (
    <div className="mega-card flex flex-wrap items-center gap-2 p-3">
      ...
      <select
        ...
        className="rounded-md border border-mega-gray-200 px-2.5 py-1.5 text-xs text-mega-gray-700"
        title="Ao escolher uma loja, a regional é detectada automaticamente"
      >
