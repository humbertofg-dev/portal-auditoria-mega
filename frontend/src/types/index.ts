...
export type StatusControleDiario =
...
export interface ItemControleDiario {
  id: string;
  controleDiarioId: string;
  lojaId: string;
  loja: Loja & { regional: Regional | null };
...
export interface ControleDiario {
  ...
  fechadoPor?: { id: string; nome: string } | null;
  itens: ItemControleDiario[];
  _count?: { itens: number };
}

export interface DashboardControleDiario {
  total: number;
  recebidas: number;
  concluidas: number;
  emAnalise: number;
  ...
  percentual: number;
  valorTotal: number;
  tempoMedioEnvioMin: number | null;
  tempoMedioAnaliseMin: number | null;
  todasEnviaram: boolean;
  fechado: boolean;
}

export interface HistoricoControleDiario {
  id: string;
  dataControle: string;
